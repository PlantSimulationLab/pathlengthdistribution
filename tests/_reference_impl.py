        ...
        for i in range(0, N):
            ...
            while ze <= z_max:
                ...
                if shape == 'prism':
                    ...
                    raise Exception('Invalid shape argument. Options are: prism, ellipsoid, cylinder, polymesh')

                if (dr > kEpsilon):
                    ray_hit += 1

                #print("ray_hit ",ray_hit)
            ...
            #print("ray hits: ", ray_hits)

    path_length = np.array(path_length, dtype=float)

    projected_area = np.sum(path_length>kEpsilon)/(N*N)*bbox_sizex*bbox_sizey*cos(ray_zenith)
...
def pathlengthdistribution(shape, scale_x, scale_y, scale_z, ray_zenith, ray_azimuth, nrays, plyfile='', bins=10, normalize=True):
    ...
    path_lengths,_ = pathlengths(shape, scale_x, scale_y, scale_z, ray_zenith, ray_azimuth, nrays, plyfile)
...
def crownabsorptionfraction(Gtheta, LAD, shape, scale_x, scale_y, scale_z, ray_zenith, ray_azimuth, nrays, plyfile=''):

    path_length, Ap = pathlengths(shape, scale_x, scale_y, scale_z, ray_zenith, ray_azimuth, nrays, plyfile)

    fabs = 0
    for i in range(path_length.size):
        fabs = fabs + (1-np.exp(-Gtheta*LAD*path_length[i]))

    fabs = fabs/path_length.size/Ap

    return fabs


def crownsunlitfraction(Gtheta, LAD, shape, scale_x, scale_y, scale_z, ray_zenith, ray_azimuth, nrays, plyfile=''):
    path_length, Ap = pathlengths(shape, scale_x, scale_y, scale_z, ray_zenith, ray_azimuth, nrays, plyfile)
